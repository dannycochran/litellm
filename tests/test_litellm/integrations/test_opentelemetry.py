...
class TestOpenTelemetryExternalSpan(unittest.TestCase):
    ...
    def test_external_span_not_closed_with_use_otel_litellm_request_span_true(self):
        ...
        # Load test data — each "completion" gets its own kwargs to match
        # production, where the per-handler dedupe in _mark_success_span_once
        # is scoped to a single request's kwargs dict (Bugfix #3).
...
class TestOpenTelemetryProxyParentSpanChildEmission(unittest.TestCase):
    """Bugfix #1: When metadata includes litellm_parent_otel_span (the proxy
    span), the primary litellm_request span must still be created as a child
    so the trace hierarchy is complete. Previously _get_span_context returned
    the parent span object, which made should_create_primary_span evaluate
    False and the child span was never emitted."""
...
class TestOpenTelemetryProxyLoggerFirstRegisteredWins(unittest.TestCase):
    """Bugfix #2: open_telemetry_logger ownership must not be silently
...
class TestOpenTelemetrySuccessSpanDedupe(unittest.TestCase):
    """Bugfix #3: streaming code paths trigger both sync and async success
...
class TestOpenTelemetryFailureSpanDedupe(unittest.TestCase):
    """Bugfix #3 (failure-side): streaming code paths trigger both sync and
    async failure callbacks for one request, causing duplicate ERROR
    litellm_request spans. _mark_failure_span_once provides a request-local
